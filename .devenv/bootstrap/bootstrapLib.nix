...
{inputs}: rec {
  ...
  getOverlays = inputName: inputAttrs: let
  ...
  mkDevenvForSystem = {
        ...
        config =
          ...
          // {
            allowUnfreePredicate =
              if nixpkgs_config.allowUnfree or false
              then (_: true)
              else if (nixpkgs_config.permittedUnfreePackages or []) != []
              then (pkg: builtins.elem (lib.getName pkg) (nixpkgs_config.permittedUnfreePackages or []))
              else (_: false);
            allowInsecurePredicate =
              if (nixpkgs_config.permittedInsecurePackages or []) != []
              then (pkg: builtins.elem (lib.getName pkg) (nixpkgs_config.permittedInsecurePackages or []))
              else (_: false);
          }
    ...
    tryImport = resolvedPath: basePath:
      if lib.hasSuffix ".nix" basePath
      ...
      in
        ...
        then [(import devenvpath)] ++ lib.optional (builtins.pathExists localpath) (import localpath)
        else throw (basePath + "/devenv.nix file does not exist");

    importModule = path:
      if lib.hasPrefix "path:" path
      then
    ...
    mkCommonModules = evalPkgs:
        ...
        (
          {options, ...}: {
            config.devenv = lib.mkMerge [
              ...
              (
                if builtins.hasAttr "cli" options.devenv
                then {
                  cli.version = version;
      ...
      ++ (
        if !skip_local_src
        then (importModule (devenv_root + "/devenv.nix"))
        else []
      )
      ++ [
        (
