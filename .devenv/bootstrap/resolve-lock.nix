# Adapted from https://git.lix.systems/lix-project/flake-compat/src/branch/main/default.nix
{
    ...
    (
      key: node: let
        sourceInfo =
          ...
          else if node.locked.type or null == "path" && node.locked.path or null == "."
          then rootSrc
          else let
            locked = node.locked;
        ...
        flake =
          if key == lockFile.root
          then null
          else import (outPath + "/flake.nix");

        outputs =
          if key == lockFile.root
          then {}
          else flake.outputs (inputs // {self = result;});
      ...
      in
        if node.flake or true && key != lockFile.root
  ...
  result =
    ...
    then throw "${lockFilePath} does not exist"
